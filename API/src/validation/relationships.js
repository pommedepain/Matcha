...
class RelationshipValidator {
  ...
  validateNodea() {
    return new Promise((resolve) => {
      if (this.req.node_a && this.data.node_a.type === 'User') {
        resolve(new UserValidator({ username: true }, this.data.noda_a.value).validate())
  ...
  validateNodeb() {
    return new Promise((resolve) => {
      if (this.req.node_b && this.data.node_b.type === 'User') {
        resolve(new UserValidator({ username: true }, this.data.noda_b.value).validate())
      } else if (this.req.node_b && this.data.node_b.type === 'Tag') {
        resolve(new TagValidator({ id: true }, this.data.noda_b.value).validate())
      } else resolve(true);
    });
  }
  ...
  validateRelationship() {
    return new Promise((resolve, reject) => {
      ...
      if (this.req.relation) sch.relation = Joi.string().regex(/^[a-zA-Z-_]{3,30}$/).required();
      else sch.relation = Joi.string().regex(/^[a-zA-Z-_]{3,30}$/);
